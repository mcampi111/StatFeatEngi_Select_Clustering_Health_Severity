...
km.res.param <- lapply(1:length(datacluster_nonparam), function(i)
                  lapply(1:length(datacluster_nonparam[[i]]), function(j) 
...
rbind(do.call(rbind, avg_sil_param[[1]]), do.call(rbind, avg_sil_param[[2]]),
      do.call(rbind, avg_sil_param[[3]]), do.call(rbind, avg_sil_param[[4]]) )


lapply(1:length(datacluster_param), function(i)
  lapply(1:length(datacluster_param[[i]]), function(j) 
    fviz_silhouette(sil_kmeans_param[[i]][[j]])[[1]][[1]][[1]]  ))
...
rbind(do.call(rbind, avg_sil_nonparam[[1]]), do.call(rbind, avg_sil_nonparam[[2]]),
      do.call(rbind, avg_sil_nonparam[[3]]), do.call(rbind, avg_sil_nonparam[[4]]) )


#confusion matrix 1
# table(km.res.param$cluster,param_scaled$Type2)
# table(param_scaled$Type2, km.res.param$cluster)
# 
# 
# cluster_labels <- levels(param_scaled$Type2) #I NEED TO CHECK MORE HER
# results_kmeans = as.factor(cluster_labels[km.res.param$cluster])
# results_kmeans <- factor(results_kmeans, levels = levels(param_scaled$Type2))
# 
# confusionMatrix(data = results_kmeans, reference = param_scaled$Type2)
# 
# #confusion matrix 2
# 
# # True class labels
# true_labels <- param_scaled$Type2
# # Cluster assignments
# cluster_labels <- km.res.param$cluster
...
# cluster_labels_mapped <- factor(cluster_to_class_mapping[cluster_labels],levels = levels(param_scaled$Type2))
...
################################
param_hierarhical <- as.data.frame(param_scaled[,c(1:12)]) 
rownames(param_hierarhical) <- sapply(1:nrow(param_scaled), 
                                 function(i) paste(param_scaled$Type2[i], i, sep = "_"))



res.hc.param = eclust(param_hierarhical, "hclust", k = 4)

fviz_dend(res.hc.param,
...
nonparam_hierarhical <- as.data.frame(nonparam_scaled[,c(1:12)]) 
rownames(nonparam_hierarhical) <- sapply(1:nrow(nonparam_scaled), 
                                      function(i) paste(nonparam_scaled$Type2[i], i, sep = "_"))



res.hc.nonparam = eclust(nonparam_hierarhical, "hclust", k = 4)

fviz_dend(res.hc.nonparam,
          k = 4, # Cut in four groups
          cex = 0.5, # label size
          k_colors = c("orange", "green", "pink", "gold"),
          color_labels_by_k = TRUE, # color labels by groups
...
fviz_silhouette(res.hc.param, palette = c("green", "yellow","pink","orange" ) ) + theme_bw() + 
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        legend.position = 'right',
        axis.text=element_text(size=5, angle = 90)) #+ 
...
fviz_silhouette(res.hc.nonparam, palette = c("green", "yellow","pink","orange" ) ) + theme_bw() + 
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        legend.position = 'right',
        axis.text=element_text(size=5, angle = 90)) #+ 
...
# #confusion matrix 1
# clusters =cutree(res.hc.param, k=4)
# table(param_scaled$Type2,clusters)
# 
# 
#confusion matrix 2
# True class labels
...
silhouette(cluster_labels, dist(param_scaled[,c(1:12)]))



## Determining the optimal number of clusters ##
res.hc.param <- param_scaled[,c(1:12)] %>%
  scale() %>%
