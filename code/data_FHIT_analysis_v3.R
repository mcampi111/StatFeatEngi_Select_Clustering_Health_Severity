...
colnames(t_test_mean_diff_bl) <- c("Type1", "Type2",colnames(data_FHIT3_black_ctr[5:10]))
t_test_mean_diff_bl
...
colnames(w_test_mean_diff_bl) <- c("Type1", "Type2",colnames(data_FHIT3_black_ctr[5:10]))
w_test_mean_diff_bl
...
colnames(test_cmv_bl) <- c("Type1", "Type2",colnames(data_FHIT3_black_ctr[5:10]))
test_cmv_bl
...
# Covariance
cov_test_bl = as.data.frame(cbind(combo_category,do.call(rbind,lapply(1:6, function(j) 
                               round(testCov(data_FHIT3_black[,c(5:10,3)] %>% 
                                  filter(Type == combo_category[j,1] ) 
                                %>% dplyr::select(-ncol(.)),
                                data_FHIT3_black[,c(5:10,3)] %>% 
                                  filter(Type == combo_category[j,2] ) %>% 
                                  dplyr::select(-ncol(.)))[[1]]$p.value,3))) ) )

cov_test_bl = cov_test_bl %>% mutate_at(c('V3'), as.numeric)
colnames(cov_test_bl) <- c("Type1", "Type2", "Significance")
cov_test_bl
...
stat_dec_cmv_bl$Test = rep('CMV', dim(stat_dec_cmv_bl)[1])
...
stat_dec_cov_bl = do.call(rbind,
                           lapply(1:dim(id_cov_bl)[1], 
                                  function(i) 
                                    cov_test_bl[c(2,3,6),][ id_cov_bl[i,1],c(1:2) ]))

stat_dec_cov_bl$Discr_Var = rep('NA', dim(stat_dec_cov_bl)[1])
...
#better format
##############
final_dec_bl2[18,1] = "L. Post. - R. Ant."
final_dec_bl2[19,1] = "R. Post. - R. Ant."
final_dec_bl2[20,1] = "L. Lat. - L. Post."
final_dec_bl2[21,1] = "L. Ant. - R. Ant."
final_dec_bl2[22,1] = "L. Lat. - R. Lat."
...
pdf( paste(figs_dir, "heat_black.pdf", sep = ""), width = 12, height = 9 )  
...
mean_boot_adhd_bl_r_l = boot(data_FHIT3_black_adhd$`R. Lat.`,
                             statistic = meanfun, sim = "parametric",
                             R=25)
...
var_boot_adhd_bl_l_l = boot(data_FHIT3_black_adhd$`L. Lat.`,
                            statistic = varfun, sim = "parametric",
                            R=25)
...
tosample$Type = c('mean','mean','mean','mean',
                  ...
                  "cov_multi", "cop_multi", "cop_multi", "cop_multi", "cop_multi", "cop_multi")
tosample =  as.data.frame(unique(cbind(tosample$V1,tosample$Type)))
colnames(tosample) = c("variable", "Type")

tosample$stat = c("meanfun","meanfun","varfun", rep(NA, 12))
tosample


list_cat_child = list(data_FHIT3_black_adhd,
                      data_FHIT3_black_ctr,
                      data_FHIT3_black_dys,
                      data_FHIT3_black_tsa)

new_data_nonparm = as.data.frame( do.call(cbind , 
                       lapply(1:3, function(i)  
                         do.call(rbind,map(list_cat_child, ~ boot(data = .x[[ tosample$variable[i]]],
                           statistic = get(tosample$stat[i]), 
                           R = 25)$t) ) ) ) )


#fix this apply(tosample[,-ncol(tosample)], 1, function(row) paste(row, collapse = "_"))
colnames(new_data_nonparm) = c("Mean_RLat", "Mean_LAnt", "Var_RLat"  )
new_data_nonparm$Type = c(sapply(1:4, function(i) rep(unique(list_cat_child[[i]]$Type), 25 )))
new_data_nonparm$color = new_data_bl$color
...
#by experiments
new_data_bl_unique<- new_data_nonparm    # cannot in the case of new_data_bl
new_data_bl_unique <- unique(new_data_bl_unique)

tsne_bl <- Rtsne(new_data_bl_unique[,c(1:3)], 
                 dims = 2,
                 pca = TRUE,
                 perplexity = 10)
...
head(new_data_nonparm) #new_data_bl


new_data_nonparm = new_data_nonparm %>%  #new_data_bl
  mutate(
    # Create categories
    Type2 = dplyr::case_when(
      Type == "TSA"  ~ "ASD",
...
data_bl_scaled <- new_data_nonparm %>% mutate_at(c(1:3), funs(c(scale(.)))) #new_data_bl - 4


res.dist.bl <- get_dist(data_bl_scaled[,c(1:3)], stand = TRUE, method = "pearson") # - 4
...
data_bl_kmeans = data_bl_scaled[,c(1:3)]
...
sil_kmeans <- silhouette(km.res.black$cluster, dist(data_bl_kmeans))
rownames(sil_kmeans) <- sapply(1:nrow(data_bl_scaled), 
                                        function(i) paste(data_bl_scaled$Type2[i], i, sep = "_"))
head(sil_kmeans[, 1:3])
...
cluster_colors = c('orange', 'pink', 'yellow', 'green')

#need to find a way here - not sure
#color_var <- data_bl_scaled$Type2
#
# myfviz_silhouette(sil_kmeans, palette = cluster_colors,
#                   color_var, 
#                   label=TRUE
#                  ) +
#   theme_bw() + 
...
pdf( paste(figs_dir, "sil_kmeans.pdf", sep = ""), width = 12, height = 9 )  
fviz_silhouette(sil_kmeans, palette = cluster_colors) +
  theme_bw() + 
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        legend.position = 'right',
...
data_bl_hier <- as.data.frame(data_bl_scaled[,c(1:3)]) #new_data_bl
rownames(data_bl_hier) <- sapply(1:nrow(new_data_bl), 
                                 function(i) paste(new_data_bl$Type2[i], i, sep = "_"))



res.hc.black = eclust(data_bl_hier, "hclust", k = 4)
...
pdf( paste(figs_dir, "sil_hclust.pdf", sep = ""), width = 12, height = 9 )  
fviz_silhouette(res.hc.black, palette = c("green", "pink","orange","yellow" ) ) + theme_bw() + 
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        legend.position = 'right',
        axis.text=element_text(size=5, angle = 90)) #+ 
...
silhouette(cluster_labels, dist(data_bl_scaled[,c(1:3)]))



## Determining the optimal number of clusters ##

res.nbclust.black <- new_data_bl[,c(1:3)] %>%
  scale() %>%
...
corMat_FHIT3_black <- cor(new_data_bl[,c(1:3)])
...
fviz_contrib(res.pca.FHIT3.black, choice = "var", axes = 2, top = 10)
