...
colnames(test_mean_diff_bl) <- c("Type1", "Type2",colnames(data_FHIT3_rotfram_ctr[5:10]))
test_mean_diff_bl
...
colnames(test_var_ratio_bl) <- c("Type1", "Type2",colnames(data_FHIT3_rotfram_ctr[5:10]))
test_var_ratio_bl
...
colnames(test_kolmo_bl) <- c("Type1", "Type2",colnames(data_FHIT3_rotfram_ctr[5:10]))
test_kolmo_bl
...
stat_dec_copula_bl = rbind(do.call(rbind,
                           ...
                           do.call(rbind,
                                   lapply(1:length(which(result_bl_tsa_dys$tbl[,4] < 0.05, arr.ind=TRUE)),function(i)
                                     cbind(cbind("DYS", "TSA"),
                                           result_bl_tsa_dys$tbl[which(result_bl_tsa_dys$tbl[,4] < 0.05, 
                                                                       arr.ind=TRUE),][,1][i]))),
                           do.call(rbind,
                                   lapply(1:length(which(result_bl_adhd_dys$tbl[,4] < 0.17, arr.ind=TRUE)),function(i)
                                     cbind(cbind("ADHD","DYS"),
                                           result_bl_adhd_dys$tbl[which(result_bl_adhd_dys$tbl[,4] < 0.17, 
                                                                        arr.ind=TRUE),][,1][i]))) )
...
stat_dec_copula_bl$Significance = c(rep(0.01, length(which(result_bl_adhd_tsa$tbl[,4] < 0.01, arr.ind=TRUE))),
                            rep(0.05, length(which(result_bl_tsa_dys$tbl[,4] < 0.05, arr.ind=TRUE))),
                            rep(0.17, length(which(result_bl_adhd_dys$tbl[,4] < 0.17, arr.ind=TRUE))))
stat_dec_copula_bl
...
final_dec_rf2[6,1] = "Right_Post_rf-Right_Anter_rf"
final_dec_rf2[7,1] = "Left_Lateral_rf-Left_Post_rf"
final_dec_rf2[8,1] = "Right_Post_rf-Right_Anter_rf"
final_dec_rf2[9,1] = "Left_Anter_rf-Right_Anter_rf"
final_dec_rf2[10,1] = "Right_Post_rf-Left_Anter_rf"
final_dec_rf2
...
ggplot(data = prova, aes(x = factor(Combination),
  ...
  scale_fill_gradient2("Significance",
                       low = "cyan", mid = "white", high = "blue",breaks=c(0.01,0.1,0.15,0.25),
                       guide = "legend") + 
...
final_dec_rf$Combination = paste(final_dec_bl$Type1,'-',final_dec_bl$Type2)
